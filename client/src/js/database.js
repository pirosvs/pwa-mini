...
export const postDb = async (name, home, cell, email)  => {
    ...
    const request = store.add({ name: name, home: home-phone, cell: cell-phone, email: email});
    // needs to take in name, home, cell, email
...
export const getDb = async () => {
  ...
  const request = store.getAll();
    //  const request = store.get(id); if only for one
  const result = await request;
